...
struct rec_byte_time_flags {
  ...
  byte reserved2:1; // (minut) Ежеминутный будильник будет специальной отдельной записью (минуты проверяем всегда)
  byte hour:1;
  byte dayOfMonth:1;
  byte month:1;
  byte year:1;
...
struct rec_alarm { // 24 байта
  struct rec_data_time alarm_time; // 6 байт
  struct rec_data_time last_alarm_time; // 6 байт
